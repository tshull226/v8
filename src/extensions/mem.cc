#include <cstring>
//TODO Need to replace necessary functions correctly
...
void Mem::Initialize(Handle<Object> target) {
  ...
  WEBCUDA_SET_METHOD(target, "memAlloc", Mem::Alloc);
  WEBCUDA_SET_METHOD(target, "memAllocPitch", Mem::AllocPitch);

  constructor_template->InstanceTemplate()->SetAccessor(String::New("devicePtr"), Mem::GetDevicePtr);

  WEBCUDA_SET_PROTOTYPE_METHOD(constructor_template, "free", Mem::Free);
  WEBCUDA_SET_PROTOTYPE_METHOD(constructor_template, "copyHtoD", Mem::CopyHtoD);
  WEBCUDA_SET_PROTOTYPE_METHOD(constructor_template, "copyDtoH", Mem::CopyDtoH);
}
...
Handle<Value> Mem::GetDevicePtr(Local<String> property, const AccessorInfo &info) {
  ...
  return scope.Close(ptrbuf->handle_);
	*/
	return scope.Close(Number::New(error)); //totally bogus
